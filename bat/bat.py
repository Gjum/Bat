...
def run_task(task, event_plugin, callback=None):
	...
	- subscribe to events that are generated by the task
	- send the event data to the generator
	...
	:return: first generated value of the generator
	"""
	def register(response):
		non_events = (True, False, None)
		if response in non_events:
			logger.warn('[Task] Not subscribing to non-event response: %s', type(response))
			handler(response, {'response': response})
		else:
			event_plugin.reg_event_handler(response, handler)

	def handler(evt, data):
		try:
			response = task.send(data)
